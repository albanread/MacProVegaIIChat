...
A local chat, reading and drafting app for the **Mac Pro (2019) with a Radeon Pro Vega II** —
one of the machines that every "just run an LLM locally" guide tells you to replace.

Scope, stated plainly: this was built and tested on **one machine** of exactly that
description. It is not a claim about AMD GPUs in general, or even about other Vega IIs.

Download a model, press Start, and talk to it. Drop a document on the window and it will
read it. Press ⌘D and it will write one. Everything happens on the machine; nothing
leaves it.

![minimum macOS 13.0](https://img.shields.io/badge/macOS-13.0%2B-lightgrey) ![x86_64](https://img.shields.io/badge/arch-x86__64-blue)
...
**Tested on: one Mac Pro 7,1 with a Radeon Pro Vega II (32 GB), macOS 26.3.**

That is the whole of the evidence. Everything else in this repo is scoped to it. A
different Vega II, a W5700X, a W6800X, a different macOS — all untested. The underlying
fixes are about wave64 SIMD width rather than one specific chip, so there is reason to
think they generalise, but reason-to-think is not testing and we are not going to dress
it up as support. If you try it on other hardware, please report what happens.
